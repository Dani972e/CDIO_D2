...
public class FxApp extends Application {
		...
		try {
			...
			userInput.caretPositionProperty().addListener(new ChangeListener<Number>() {
				...
				public void changed(ObservableValue<? extends Number> observable, Number oldValue, Number newValue) {
					/*
					 * when button is pressed, the textfield looses focus and
					 * caret is moved to 0. Hence the previousCaretPosition is
					 * the actual position.
					 */
					// System.out.println("onChange "+oldValue+" -->
			...
			for (int i = 0; i < 10; i++) {
				...
				btnnum[i].setOnAction(new EventHandler<ActionEvent>() {
					@Override
					public void handle(ActionEvent event) {
						onNumBtnPressed(inputHandler, btn);
					}
				});
			}
	...
	private void onNumBtnPressed(final FxAppInputBtnHandler inputHandler, final int btn) {
		final char c = inputHandler.onButtonPressed(btn, inputType, DELAY);
		if (timer == null)
			timer = new Timer();
		else {
			timer.cancel();
